...
- **Repo renamed `ethora-mcp-cli` → `ethora-mcp-server`** to match the npm package (`@ethora/mcp-server`) and the MCP ecosystem's vocabulary — a stdio MCP server is still an "MCP server" regardless of running locally. The registry `mcpName` becomes `io.github.dappros/ethora-mcp-server`; `package.json` / `server.json` / `.plugin/plugin.json` repo URLs updated to match. The npm package name is unchanged. (The old `io.github.dappros/ethora-mcp-cli` registry entry from 26.5.1/26.5.2 is now superseded.)
